...
static void PrintDeviceInfo(MV_CC_DEVICE_INFO *mv_dev_info)
{
  ...
  {
    printf("UserDefinedName: %s", mv_dev_info->SpecialInfo.stUsb3VInfo.chUserDefinedName);
    printf("Serial Number: %s", mv_dev_info->SpecialInfo.stUsb3VInfo.chSerialNumber);
    printf("Device Number:  %d", mv_dev_info->SpecialInfo.stUsb3VInfo.nDeviceNumber);

    // TODO：BUG修复
    //  SPDLOG_INFO("UserDefinedName: {}.", mv_dev_info->SpecialInfo.stUsb3VInfo.chUserDefinedName);
    //  SPDLOG_INFO("Serial Number: {}.", mv_dev_info->SpecialInfo.stUsb3VInfo.chSerialNumber);
    //  SPDLOG_INFO("Device Number: {}.", mv_dev_info->SpecialInfo.stUsb3VInfo.nDeviceNumber);
  }
  else
  {
...
bool HikCamera::OpenPrepare(unsigned int index)
{
  ...
  {
    SPDLOG_ERROR("Intput index:{} >= nDeviceNum:{} !", index, mv_dev_list_.nDeviceNum);
    return false;
  }
